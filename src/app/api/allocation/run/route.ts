...
export async function POST(request: Request) {
  try {
    ...
    if (
      typeof nonReservedMemberCount !== "number" ||
      !Number.isInteger(nonReservedMemberCount) ||
    ...
    const preview = await buildAllocation({
      nonReservedMemberCount,
      eventDate: event.date,
    });

    // Make sure the selected event belongs to the same guild
    // used by the allocation engine.
    if (preview.guildId !== event.guildId) {
      return NextResponse.json(
        {
          error:
    ...
    for (const resource of guild.resources) {
      const currentIndex =
        resource.rotationStates[0]?.rotationIndex ??
        0;

      rotationBefore[resource.id] =
        currentIndex;
      ...
      const selectedCount =
        resourceResult?.selectedMembers.length ??
        0;
    ...
    return NextResponse.json({
      ...
      bidPages: {
        feathers: Math.ceil(
          bidSlotsByType.FEATHER.length /
            SLOTS_PER_PAGE
        ),

        cards: Math.ceil(
          bidSlotsByType.CARD.length /
            SLOTS_PER_PAGE
        ),
