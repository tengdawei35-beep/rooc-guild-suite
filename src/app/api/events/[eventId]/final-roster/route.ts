...
import { notifyRosterPublished } from "@/lib/discord-roster-notification";
...
export async function PUT(request: Request, context: RouteContext) {
  try {
    ...
    const event = await prisma.event.findFirst({
      ...
      select: { id: true, guildId: true, type: true, date: true, finalRosterId: true },
    });
    ...
    const wasAlreadyPublished = event.finalRosterId === roster.id;
    ...
    // Editing, saving, generating, and unpublishing rosters are silent.
    // Re-publishing the same already-published roster is also silent.
