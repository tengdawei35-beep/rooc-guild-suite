...
export async function POST(request: Request) {
  try {
    ...
    if (!planId || !guildName || !discordGuildId) return NextResponse.json({ error: "PLAN_AND_GUILD_DETAILS_REQUIRED" }, { status: 400 });
    ...
    const session = await stripePaymentProvider.createCheckout({
      planId: plan.id,
      stripePriceId,
      customer: { discordUserId: user.discordId, username: user.username },
