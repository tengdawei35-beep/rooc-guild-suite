...
export async function GET() {
  try {
    ...
    const [members, events, rosters, resources, reservations, allocationRuns, rosterMembers] = await Promise.all([
      ...
      prisma.rosterMember.findMany({ where: { party: { roster: { event: { guildId } } } }, select: { memberId: true, attendance: true } }),
    ]);
    ...
    const attended = rosterMembers.filter((member) => member.attendance === true).length;
    const attendanceEvents = rosterMembers.filter((member) => member.attendance !== null).length;
    const attendanceRate = attendanceEvents ? (attended / attendanceEvents) * 100 : 0;
    ...
    return NextResponse.json({
      ...
      attendance: { events: attendanceEvents, attended, rate: Number(attendanceRate.toFixed(1)) },
