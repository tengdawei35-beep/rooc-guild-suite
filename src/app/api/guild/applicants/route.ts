...
async function buildComparison(guildId: string, applicant: Awaited<ReturnType<typeof prisma.guildApplicant.findFirst>>) {
  ...
  const members = await prisma.guildMember.findMany({
    ...
    select: { id: true, characterName: true, ...MEMBER_STATS_SELECT },
  });

  const scoredMembers = scoreRooPlayers(members);
  const scored = scoreRooPlayers([
    ...members,
    {
      ...
      pdef: applicant.pdef,
      mdef: applicant.mdef,
      pvpDamageBonus: applicant.pvpDamageBonus,
      pvpDamageReduction: applicant.pvpDamageReduction,
      pdmgPercent: applicant.pdmgPercent,
      mdmgPercent: applicant.mdmgPercent,
      pdmgReductionPercent: applicant.pdmgReductionPercent,
      mdmgReductionPercent: applicant.mdmgReductionPercent,
      critRes: applicant.critRes,
      ignorePdef: applicant.ignorePdef,
      ignoreMdef: applicant.ignoreMdef,
      damageVsMedium: applicant.damageVsMedium,
      damageReductionVsMedium: applicant.damageReductionVsMedium,
      damageVsSmall: applicant.damageVsSmall,
      damageReductionVsSmall: applicant.damageReductionVsSmall,
      damageVsDemiHuman: applicant.damageVsDemiHuman,
      damageReductionVsDemiHuman: applicant.damageReductionVsDemiHuman,
      damageVsBrute: applicant.damageVsBrute,
      damageReductionVsBrute: applicant.damageReductionVsBrute,
      equipmentPdefPercent: applicant.equipmentPdefPercent,
      equipmentMdefPercent: applicant.equipmentMdefPercent,
      patk: applicant.patk,
      matk: applicant.matk,
      hp: applicant.hp,
    },
  ...
  const closest = findClosestRooMember(applicantScore, scoredMembers);

  return {
    scores: {
      dpsScore: applicantScore.dpsScore,
      tankScore: applicantScore.tankScore,
      pvpScore: applicantScore.pvpScore,
      dpsPercentile: applicantScore.dpsPercentile,
      tankPercentile: applicantScore.tankPercentile,
      pvpPercentile: applicantScore.pvpPercentile,
      rawPdef: applicantScore.rawPdef,
      rawMdef: applicantScore.rawMdef,
    },
    closest: closest ? {
      distance: closest.distance,
      characterName: closest.member.characterName ?? null,
      job: closest.member.job ?? null,
      dpsScore: closest.member.dpsScore,
      tankScore: closest.member.tankScore,
      pvpScore: closest.member.pvpScore,
      dpsPercentile: closest.member.dpsPercentile,
      tankPercentile: closest.member.tankPercentile,
      pvpPercentile: closest.member.pvpPercentile,
    } : null,
  };
}
...
export async function GET() {
  ...
  const applicants = await prisma.guildApplicant.findMany({
    where: { guildId: auth.guild.id },
    orderBy: { createdAt: "desc" },
  });

  const result = await Promise.all(applicants.map(async (applicant) => ({
    ...applicant,
    comparison: await buildComparison(auth.guild.id, applicant),
  })));
...
export async function POST(request: Request) {
  ...
  if (body.decision === "DENIED") {
    const updated = await prisma.guildApplicant.update({
      where: { id: applicant.id },
      data: { status: "DENIED", remarks: body.remarks?.trim() || null, decidedAt: new Date(), decidedByUserId: auth.user.id },
    });
  ...
  const created = await prisma.$transaction(async (tx) => {
    const member = await tx.guildMember.create({
      data: {
        ...
        discordUserId: applicant.discordUserId,
    ...
    await tx.guildApplicant.update({
      where: { id: applicant.id },
      data: { status: "ACCEPTED", remarks: body.remarks?.trim() || null, decidedAt: new Date(), decidedByUserId: auth.user.id },
    });
