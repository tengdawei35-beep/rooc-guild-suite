...
const FIELD_LABELS: Record<string, string[]> = {
  ...
  critRes: ["crit res", "critical resistance"],
  ignorePdef: ["ignore pdef", "ignore physical defense"],
  ignoreMdef: ["ignore mdef", "ignore magic defense"],
...
function extractFields(text: string): Extracted {
  ...
  for (const line of lines) {
    for (const [field, labels] of Object.entries(FIELD_LABELS)) {
      const label = labels.find((candidate) => line.startsWith(candidate) || line.includes(`${candidate}:`));
      if (!label) continue;

      const after = line.slice(line.indexOf(label) + label.length).replace(/^\s*[:=-]?\s*/, "");
      if (!after) continue;

      if (NUMERIC_FIELDS.has(field)) {
        const match = after.replace(/,/g, "").match(/-?\d+(?:\.\d+)?/);
  ...
  return (
    <div className="space-y-5 rounded-xl border border-zinc-800 bg-zinc-950/60 p-5">
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-400">Upload one or more clear game-stat screenshots. OCR runs locally in your browser, then fills the matching member fields for review.</p>
      </div>
      ...
      <button type="button" onClick={runOcr} disabled={busy || !files.length} className="rounded-lg bg-white px-4 py-2 text-sm font-medium text-black disabled:cursor-not-allowed disabled:opacity-40">
        {busy ? `Reading screenshots… ${progress}%` : "Read screenshots"}
      </button>
      ...
      {Object.keys(fields).length > 0 && (
        <div className="rounded-lg border border-emerald-900/50 bg-emerald-950/20 p-4">
          <p className="text-sm font-medium text-emerald-300">Detected {Object.keys(fields).length} fields. The values have been placed into the member form for review.</p>
        </div>
      )}
      ...
      {text && (
        <details>
          ...
          <pre className="mt-2 max-h-48 overflow-auto whitespace-pre-wrap rounded-lg bg-zinc-900 p-3 text-xs text-zinc-500">{text}</pre>
        </details>
      )}
    </div>
  );
