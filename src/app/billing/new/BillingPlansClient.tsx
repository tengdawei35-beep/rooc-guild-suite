...
export default function BillingPlansClient({ plans, ownedGuildCount, guilds, complimentary }: { plans: Plan[]; ownedGuildCount: number; guilds: Guild[]; complimentary: Complimentary | null }) {
  ...
  async function submit(path: string) {
    ...
    setLoading(true);
    try {
      const response = await fetch(path, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ planId: selectedPlan, ...(renewing ? { guildId: selectedGuildId } : { guildName: guildName.trim(), discordGuildId: discordGuildId.trim() }) }) });
      const data = await response.json() as { checkoutUrl?: string; success?: boolean; guildId?: string; error?: string };
