...
type Existing = Partial<Stats> & { id: string; characterName: string; job: string | null };
...
export default function ApplicantApplyClient({
  token,
  discordUserId,
  discordUsername,
  existingApplication,
  ...
  async function submit() {
    ...
    try {
      const response = await fetch(`/api/apply/${token}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ applicationId: existingApplication?.id, characterName: characterName.trim(), job: job.trim(), ...stats }),
      });
      const data = await response.json();
  ...
  return <main className="min-h-screen bg-zinc-950 p-6 text-white"><div className="mx-auto max-w-6xl py-8">
    ...
    <section className="mb-6 grid gap-4 md:grid-cols-3">
      <Info label="Discord User ID" value={discordUserId} />
      <Info label="Discord Username" value={discordUsername} />
      <Info label="RAW PDEF" value={rawPdef.toFixed(2)} />
    </section>
    <section className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6">
      <div className="grid gap-4 md:grid-cols-2"><label className="text-sm text-zinc-400">Character Name<input value={characterName} onChange={(e) => setCharacterName(e.target.value)} className="mt-2 w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-white" /></label><label className="text-sm text-zinc-400">Job<input value={job} onChange={(e) => setJob(e.target.value)} placeholder="e.g. High Wizard" className="mt-2 w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-white" /></label></div>
      <div className="mt-8 border-t border-zinc-800 pt-6"><h2 className="font-semibold">ROO screenshots</h2><p className="mt-1 text-sm text-zinc-500">Up to 12 images, 8 MB each.</p><input className="mt-4 block w-full text-sm text-zinc-400" type="file" accept="image/png,image/jpeg,image/webp" multiple onChange={(e) => setFiles(Array.from(e.target.files ?? []))} /><button onClick={readScreenshots} disabled={reading} className="mt-4 rounded-lg bg-white px-4 py-2.5 text-sm font-medium text-black disabled:opacity-50">{reading ? "Reading screenshots…" : "Read screenshots"}</button></div>
      <div className="mt-8 border-t border-zinc-800 pt-6"><div className="flex items-center justify-between"><h2 className="font-semibold">Review stats</h2><span className="text-xs text-zinc-500">Edit any OCR result before submitting.</span></div><div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{FIELDS.map(([key, label]) => <label key={key} className="text-xs text-zinc-500">{label}<input value={stats[key]} onChange={(e) => setStats((current) => ({ ...current, [key]: e.target.value }))} className="mt-1 w-full rounded-lg border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm text-white" /></label>)}</div></div>
      {rawText && <details className="mt-6 border-t border-zinc-800 pt-6"><summary className="cursor-pointer text-sm text-zinc-400">Show raw OCR output</summary><pre className="mt-3 max-h-96 overflow-auto whitespace-pre-wrap rounded-lg bg-zinc-950 p-4 text-xs text-zinc-500">{rawText}</pre></details>}
      {error && <p className="mt-6 rounded-lg border border-red-900 bg-red-950/40 p-3 text-sm text-red-300">{error}</p>}{message && <p className="mt-6 rounded-lg border border-emerald-900 bg-emerald-950/40 p-3 text-sm text-emerald-300">{message}</p>}
      <div className="mt-8 flex justify-end border-t border-zinc-800 pt-6"><button onClick={submit} disabled={submitting} className="rounded-lg bg-white px-5 py-3 font-medium text-black disabled:opacity-50">{submitting ? "Submitting…" : "Submit application"}</button></div>
    </section>
  </div></main>;
}
