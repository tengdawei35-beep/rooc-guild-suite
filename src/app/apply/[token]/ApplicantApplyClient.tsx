...
const JOBS = [
  "High Wizard", "Professor", "High Priest", "Priest", "Lord Knight", "Paladin", "Sniper", "Assassin Cross",
  "Whitesmith", "Biochemist", "Bard", "Gypsy", "Doram (Magic)", "Doram (Support)", "Biochemist (Plant)", "Other",
];
...
export default function ApplicantApplyClient({ token, guildId, discordUserId, discordUsername, existingApplication }: { token: string; guildId: string; discordUserId: string; discordUsername: string; existingApplication: Existing | null }) {
  ...
  const hasSubmitted = Boolean(rawText) || Object.values(stats).some((value) => value !== "");
  ...
  async function submit() {
    ...
    if (!hasSubmitted) return setError("Read your ROO screenshots before submitting.");
