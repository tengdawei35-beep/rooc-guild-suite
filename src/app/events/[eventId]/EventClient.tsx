    ...
    <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            {editing && <span className="rounded-full bg-blue-950 px-2.5 py-1 text-xs text-blue-400">Editing</span>}
          </div>
          <p className="mt-1 text-sm text-zinc-600">{roster.generationMode === "AUTOMATIC" ? "Automatic" : "Manual"}</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <span className="rounded-full border border-zinc-700 bg-zinc-950 px-3 py-1 text-xs text-zinc-500">{roster.memberCount} members</span>
          <span className="rounded-full border border-zinc-700 bg-zinc-950 px-3 py-1 text-xs text-zinc-500">{roster.partyCount} parties</span>
          {canEdit && (
            <>
              <button type="button" onClick={onEdit} className={`rounded-lg border px-3 py-2 text-xs font-medium transition ${editing ? "border-white bg-white text-black" : "border-zinc-700 text-zinc-400 hover:border-zinc-500 hover:text-white"}`}>
                {editing ? "Done Editing" : "Edit Roster"}
              </button>
              <button type="button" onClick={onDeleteRoster} className="rounded-lg border border-red-900/60 px-3 py-2 text-xs font-medium text-red-400 transition hover:border-red-700 hover:bg-red-950/30 hover:text-red-300">Delete Roster</button>
            </>
          )}
        </div>
      </div>
      ...
      {editing && (
        <div className="mt-4 rounded-lg border border-blue-900/50 bg-blue-950/20 px-4 py-3 text-xs text-blue-400">
          Drag members between slots to move or swap them. Click an empty slot to add an unassigned member. Use Remove member to take someone out of the roster. Manual changes override the automatic composition.
        </div>
      )}
      ...
      {roster.parties ? (
        <div className="mt-6 space-y-8">
          {battlefields.map((battlefield) => {
            const battlefieldParties = parties.filter((party) => party.battlefield === battlefield);
            return (
              <div key={battlefield}>
                <h4 className="mb-3 text-sm font-semibold text-zinc-300">{formatBattlefield(battlefield)}</h4>
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                  {battlefieldParties.map((party) => (
                    <PartyCard
                      key={party.id}
                      party={party}
                      canEdit={canEdit}
        ...
        <div className="mt-5 flex gap-6 text-sm">
          <div><p className="text-zinc-600">Parties</p><p className="mt-1 font-semibold">{roster.partyCount}</p></div>
          <div><p className="text-zinc-600">Members</p><p className="mt-1 font-semibold">{roster.memberCount}</p></div>
          <div><p className="text-zinc-600">Created</p><p className="mt-1 font-semibold">{formatDateTime(roster.createdAt)}</p></div>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-4">
      <div className="flex items-center justify-between">
        <p className="text-sm font-semibold">Party {party.partyNumber}</p>
        <span className="text-xs text-zinc-600">{party.members.length}/5</span>
      </div>

      <div className="mt-3 space-y-2">
        {Array.from({ length: 5 }, (_, index) => {
          const slotNumber = index + 1;
          const assignment = membersBySlot.get(slotNumber);
          ...
          return (
            <div
              ...
              onDrop={(event) => {
                if (!editing) return;
                event.preventDefault();
                onDrop(party.id, slotNumber);
              }}
              className={`min-h-[58px] rounded-lg border px-3 py-2 transition ${assignment ? "border-zinc-800 bg-zinc-900" : editing ? "border-dashed border-zinc-700 bg-zinc-950 hover:border-zinc-500 hover:bg-zinc-900" : "border-transparent bg-zinc-900/40"} ${draggedAssignmentId === assignment?.id ? "opacity-40" : ""}`}
            >
              {assignment ? (
                <div
                  draggable={editing}
                  ...
                  onDragEnd={onDragEnd}
                  className={`flex items-center gap-3 ${editing ? "cursor-grab active:cursor-grabbing" : ""}`}
                >
                  ...
                  {movingAssignmentId === assignment.id && <span className="text-xs text-zinc-600">...</span>}
                </div>
              ) : editing ? (
                <button type="button" onClick={() => onEmptySlotClick(party.id, slotNumber)} className="flex min-h-[40px] w-full items-center gap-3 text-left">
                  <span className="w-4 shrink-0 text-xs text-zinc-700">{slotNumber}</span>
                  <span className="text-xs text-zinc-600 transition hover:text-white">+ Add member</span>
                </button>
              ) : (
...
function EventRules({ type }: { type: EventType }) {
  if (type === "GUILD_LEAGUE") {
    return <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Rule label="Schedule" value="Tuesday / Thursday" /><Rule label="Battlefields" value="2" /><Rule label="Players" value="40 / battlefield" /><Rule label="Parties" value="8 / battlefield" /></div>;
  }
  return <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Rule label="Schedule" value="Sunday" /><Rule label="Players" value="80" /><Rule label="Parties" value="16" /><Rule label="Party Size" value="5" /></div>;
}

function Rule({ label, value }: { label: string; value: string }) {
  return <div><p className="text-xs uppercase tracking-wider text-zinc-600">{label}</p><p className="mt-1 font-medium text-zinc-300">{value}</p></div>;
}

function StatCard({ label, value }: { label: string; value: number }) {
  return <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5"><p className="text-sm text-zinc-500">{label}</p><p className="mt-2 text-3xl font-bold">{value}</p></div>;
}

function getPartyNumber(data: EventResponse, partyId: string) {
  for (const roster of data.rosters) {
    const party = roster.parties?.find((candidate) => candidate.id === partyId);
    if (party) return party.partyNumber;
  }
  return "?";
}

function formatEventType(type: EventType) {
  return type === "GUILD_LEAGUE" ? "Guild League" : "Emperium Overrun";
}

function formatBattlefield(battlefield: "BATTLEFIELD_1" | "BATTLEFIELD_2") {
  return battlefield === "BATTLEFIELD_1" ? "Battlefield 1" : "Battlefield 2";
}

function formatEventDate(value: string) {
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return value;
  return new Intl.DateTimeFormat("en-GB", { timeZone: "Asia/Bangkok", weekday: "long", day: "numeric", month: "short", year: "numeric" }).format(date);
}

function formatDateTime(value: string) {
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return value;
  return new Intl.DateTimeFormat("en-GB", { timeZone: "Asia/Bangkok", day: "numeric", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" }).format(date);
}
